 ...
 *       Filename:  main.cu
 *
 *    Description: 	Matrix Multiplication with Unified Virtual Memory
 *
 *        Version:  1.0
...
//UVM
unsigned int *matrixA, *matrixB, *cpuOut, *gpuOut;
...
int main(){
	// Unified Memory
	cudaMallocManaged(&matrixA, A_H * A_W * sizeof(unsigned int));
	cudaMallocManaged(&matrixB, B_H * B_W * sizeof(unsigned int));
	cudaMallocManaged(&cpuOut, A_H * B_W * sizeof(unsigned int));
	cudaMallocManaged(&gpuOut, A_H * B_W * sizeof(unsigned int));
	...
	for(int i = 0; i < MAX_ITER; i++){
		...
		ckGpu->clockResume();
		gpuMatrixMul<<<gridDim, blockDim>>>(matrixA, matrixB, gpuOut, A_H, A_W, B_H, B_W);
